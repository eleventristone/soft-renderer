...
void rasterization(std::vector<Vector3f> triangle, TGAImage& framebuffer, TGAColor color, float* zBuffer) {
    ...
    for (int x = minX; x <= maxX; x++) {
        for (int y = minY; y <= maxY; y++) {
            ...
            float pab = std::sqrt((triangle[0].x * (triangle[1].y - y) + triangle[1].x * (y - triangle[0].y) + x * (triangle[0].y - triangle[1].y)) / 2.) / 2.;  // Spab = |AB x AP| / 2
            float pbc = std::sqrt((triangle[1].x * (triangle[2].y - y) + triangle[2].x * (y - triangle[1].y) + x * (triangle[1].y - triangle[2].y)) / 2.) / 2.;  // Spbc = |BC x BP| / 2
            float pac = std::sqrt((triangle[2].x * (triangle[0].y - y) + triangle[0].x * (y - triangle[2].y) + x * (triangle[2].y - triangle[0].y)) / 2.) / 2.;  // Spac = |CA x CP| / 2
